...
- Para evitar que el usuario vea un comentario como organico y luego cambie a Ads, la UI retiene comentarios recientes `post_comment` durante una ventana corta de clasificacion cuando `ads_read` esta disponible. La ventana se mide desde `inbox_items.created_at` (momento en que la app recibio/creo el item), no desde `received_at` de Meta, porque un comentario puede llegar a la app varios segundos o minutos despues de su `created_time`. En esa ventana dispara sincronizaciones Ads inmediatas y no cuenta ni muestra el item en Bandeja hasta que exista una pasada Ads completada despues del ingreso del item y hayan pasado al menos 15 segundos. Si Ads queda ocupado o no termina, el fallback maximo de retencion es 3 minutos.
