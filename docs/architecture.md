...
- Para evitar que el usuario vea un comentario como organico y luego cambie a Ads, la UI no etiqueta como organico los comentarios Facebook `post_comment` recientes mientras `ads_read` esta disponible. En su lugar muestra `Clasificando`. La ventana se mide desde `inbox_items.created_at` (momento en que la app recibio/creo el item), no desde `received_at` de Meta, porque un comentario puede llegar a la app varios segundos o minutos despues de su `created_time`. En esa ventana dispara sincronizaciones Ads inmediatas y mantiene la etiqueta neutral hasta que exista una pasada Ads completada cuyo inicio sea posterior al ingreso del item y hayan pasado al menos 15 segundos. Si Ads queda ocupado o no termina, el fallback maximo de clasificacion pendiente es 3 minutos.
