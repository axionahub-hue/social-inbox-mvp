...
- Para cuidar la cuota Free de Supabase, la UI no recarga la bandeja desde Supabase si una auto-sincronizacion no inserto ni actualizo comentarios. La carga de inbox queda limitada a los ultimos 120 items y Realtime agrupa refrescos con debounce. Los persistidores de comentarios Facebook e Instagram comparan campos relevantes antes de escribir `inbox_items`; si el comentario ya existe y no cambio, devuelven `unchanged`, no actualizan `updated_at`, no disparan Realtime y no fuerzan descarga completa del inbox.
...
- Cuando un comentario Facebook llega por webhook `Page/feed`, el backend intenta resolver en ese mismo procesamiento si el `post_id` pertenece a un anuncio Meta del workspace. Si encuentra el target Ads, persiste el comentario directamente como `ad_comment` con `provider_ad_id`; si no puede resolverlo, lo persiste como `post_comment` y las pasadas Ads posteriores pueden reclasificarlo.
- Una vez que un comentario Facebook queda clasificado como `ad_comment`, las pasadas organicas posteriores no pueden degradarlo a `post_comment` ni borrar `provider_ad_id`; Ads es la clasificacion mas especifica.
...
- Las respuestas a stories deben llegar por `entry.messaging[]`, no como comentarios publicos normales. Si Meta entrega `message.reply_to.story`, el backend las persiste como hilo privado con titulo `Respuesta a story en ...` y el mensaje queda prefijado como `Respuesta a story: ...`. Si en `webhook_events` no aparece `reply_to.story` ni ningun evento `messaging`, el problema esta en la entrega/configuracion de Meta o en ajustes de la cuenta, no en la UI.
...
- Para evitar que el usuario vea un comentario como organico y luego cambie a Ads, la UI no muestra comentarios Facebook `post_comment` recientes mientras `ads_read` esta disponible y la fuente siga pendiente. La ventana se mide desde `inbox_items.created_at` (momento en que la app recibio/creo el item), no desde `received_at` de Meta, porque un comentario puede llegar a la app varios segundos o minutos despues de su `created_time`. En esa ventana dispara sincronizaciones Ads inmediatas y mantiene el item fuera de Bandeja hasta que exista una pasada Ads completada cuyo inicio sea posterior al ingreso del item y hayan pasado al menos 15 segundos. Si Ads lo confirma, aparece como `Comentario ad`; si esa pasada no lo encuentra, aparece como `Comentario organico`. Si no hay decision luego de 2 minutos, la UI libera el comentario con el estado que tenga en ese momento.
...
- Cuando existen comentarios Facebook ocultos por clasificacion pendiente, la UI llama a `/api/meta/sync/ad-comments` con `postIds` concretos para clasificar esos posts antes del barrido general. El endpoint escanea Marketing API para encontrar esos posts entre anuncios activos/pausados y solo lee comentarios de los targets solicitados.
...
- Cada regla puede encolar like al comentario recibido, respuesta publica, respuesta privada o cualquier combinacion de esas acciones. No conversa como bot: solo ejecuta acciones fijas cuando entra un comentario que coincide.
- `automation_executions` registra una ejecucion por `rule_id`, `provider_comment_id` y destino (`like_comment`, `public_comment` o `private_message`). El indice unico evita enviar dos veces la misma automatizacion aunque el comentario llegue por webhook y polling.
...
- El apartado global de automatizaciones usa `GET /api/automation-rules?workspaceId=...` para administracion humana, no para ejecucion. Se abre como modal amplio fuera del sidebar para no comprimir el formulario. Cada fila trae metadatos minimos de la cuenta conectada para que el usuario entienda donde vive la regla.
