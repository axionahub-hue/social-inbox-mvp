...
Las acciones que dependen de Meta se aplican rapido en pantalla y quedan en segundo plano. La UI no bloquea ni muestra animaciones de envio; si Meta rechaza o no completa la accion, la conversacion vuelve a `Bandeja`, queda como no leida y muestra `Accion fallida` con el motivo.
...
- Si `ads_read` esta listo, un comentario Facebook recien recibido puede aparecer con etiqueta `Clasificando` para evitar mostrarlo como organico antes de verificar si pertenece a un anuncio. La espera empieza cuando la app recibe el item, no cuando Meta dice que fue creado. Normalmente se define despues de una pasada Ads posterior al ingreso; si Ads esta ocupado, puede quedar como `Clasificando` hasta 3 minutos como maximo de seguridad.
